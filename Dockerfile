# Usa un'immagine base con Maven e JDK 17 per la fase di build
FROM maven:3.8.5-openjdk-17 AS build

# Imposta la directory di lavoro
WORKDIR /home/app

# Copia il file pom.xml e scarica le dipendenze
COPY pom.xml .
RUN mvn dependency:go-offline

# Copia il resto del codice sorgente
COPY src ./src

# Compila l'applicazione e crea il .jar
RUN mvn package -DskipTests

# Usa un'immagine base più leggera per l'ambiente di runtime
FROM openjdk:17.0.2-slim

# Installa le dipendenze di sistema necessarie per la gestione dei font in Java AWT (usato da Apache POI).
# - libfreetype6: Libreria per il rendering dei font.
# - fontconfig: Libreria per la configurazione e la gestione dei font di sistema.
RUN apt-get update && apt-get install -y libfreetype6 fontconfig && rm -rf /var/lib/apt/lists/*

# Copia solo il .jar dalla fase di build
COPY --from=build /home/app/target/demo-0.0.1-SNAPSHOT.jar /usr/local/lib/app.jar

# Esponi la porta interna del container (verrà mappata da Cloud Run)
EXPOSE 8080

# Comando per avviare l'applicazione con la correzione per l'errore InaccessibleObjectException
ENTRYPOINT ["java", "-Dserver.port=${PORT:8080}", "--add-opens", "java.base/java.time=ALL-UNNAMED", "--add-opens", "java.base/java.time.chrono=ALL-UNNAMED", "-jar", "/usr/local/lib/app.jar"]
